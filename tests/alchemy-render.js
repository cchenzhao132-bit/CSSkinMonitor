...
// 等待 data.js（15MB+）异步加载并完成首屏渲染；jsdom 的 resources 下载是异步的，
// 固定 setTimeout 会与骨架屏 900ms 定时器形成竞态（机器慢时误报），改为轮询。
const waitReady = (w, timeoutMs) => new Promise(res => {
  const t0 = Date.now();
  const tick = () => {
    try {
      const html = w.document.getElementById('app') ? w.document.getElementById('app').innerHTML : '';
      const ready = typeof w.render === 'function' && html.indexOf('sk-line') < 0;
      if (ready || Date.now() - t0 > timeoutMs) return res(ready);
    } catch (e) {}
    setTimeout(tick, 100);
  };
  tick();
...
JSDOM.fromFile(path.join(APP, 'index.html'), {
  ...
  const ready = await waitReady(w, 20000);
  try {
    console.log('[1] 脚本加载');
    if (!ready) fail('页面 20s 内未完成首屏渲染（骨架屏未消失）');
